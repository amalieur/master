""" Sheet containing distance methods related to trajectories """
...
def _euclidean_distance(x, y):
    """
    SOURCE: https://github.com/bguillouet/traj-dist/blob/master/traj_dist/pydist/basic_euclidean.py
    Usage
    -----
...
def euclidean_dtw(t0, t1):
    """
    SOURCE: https://github.com/bguillouet/traj-dist/blob/master/traj_dist/pydist/dtw.py
    Usage
    -----
    ...
    for i in np.arange(n0) + 1:
        for j in np.arange(n1) + 1:
            C[i, j] = _euclidean_distance(t0[i - 1], t1[j - 1]) + min(C[i, j - 1], C[i - 1, j - 1], C[i - 1, j])
    dtw = C[n0, n1]
    #print(C)
    return dtw